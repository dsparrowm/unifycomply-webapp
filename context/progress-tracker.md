...
| Data | Hybrid — live auth/settings + KYC/KYB create and list/detail; lookup + TM + AML/bank mock |
...
List/detail and account purpose + start verification are wired. Next: shareholder create / tenant apps (P2).
...
3. Live KYC/KYB list/detail — **Done** (lookup + decision APIs still missing)
...
- **KYC/KYB list source (2026-09-12):** Lists are **customers** (`GET /customers/kyc|kyb`), not verification workflows. Envelope is `data: []` plus `meta.totalItems` / `totalPages` — BFF walks pages (cap 20). API statuses `pending|onboarded|review|blocked|offboarded` map to UI `pending|approved|in-review|rejected`. Assigned To stays Unassigned unless the payload includes an assignee. List rows have no documents, so Document Type is **—** until detail. Country uses `address.country` or ISO display names. Fixture detail ids (`kyc-record-*`, `kyb-record-*`) still serve Figma risk-score variants.
- **KYC/KYB detail panels (2026-09-12):** Identity, documents, and KYB shareholders come from the API when present. Risk analysis, AML, IP/device, liveness, directors, and compliance-check chrome stay on score templates — OpenAPI has no OCR/biometric/IP models. Approve/Reject/Escalate stay UI-only.
- **Pre-KYC (M2) — not located (2026-09-11):** No frame, route, nav item, feature spec, or OpenAPI tag named “Pre KYC” / “Pre-KYC” in WebApp inventory (475 frames), `design/manifest.json`, or Core Platform docs. Closest M2 candidates: (1) customer onboarding wizard `/kyc/onboarding` (MVP flowchart module **1.1** `1532:157029`, entry via Validate Document — Figma screens not exported); (2) sidebar **Packages** / **Request** placeholders under BACKGROUND CHECK (no section frames). Needs product/design confirmation of the intended Pre-KYC surface.
- **M2 API ↔ UI flows (2026-09-11):** Full comparison in `context/feature-specs/09-m2-api-ui-flow-gap.md`. API path = create customer → documents → account purpose → (KYB shareholders) → available-checks → start verification. UI path = list/detail + Perform Lookup + KYC wizard; **no** lookup/decision APIs. **Build (no/weak Figma):** account purpose, KYB intake, verification starter, shareholder create, apps selector. **Backend blockers (keep UI):** lookup/bulk, approve/reject/escalate/resubmit, rich detail panels, AML/bank/packages. **Product open:** KYC wizard business step vs KYC DTO; queue = customers vs workflows; App dropdown = `/tenants/apps` vs sandbox/production.
