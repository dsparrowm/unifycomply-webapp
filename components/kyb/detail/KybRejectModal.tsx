...
export function KybRejectModal({ open, detail, onClose, onConfirm }: KybRejectModalProps) {
  useEffect(() => {
    if (!open) {
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="kyb-reject-title"
      ...
      >
        <div className="flex items-center justify-between border-b border-[color:var(--border-default)] px-6 py-5">
          <h2 id="kyb-reject-title" className="text-lg font-semibold text-[color:var(--text-primary)]">
            Reject verification?
          </h2>
          <button
            type="button"
        ...
        >
          <p className="text-sm text-[color:var(--text-primary)]">
            ...
            <span className="font-medium">{detail.businessName}</span> ({detail.kybId}). The business
            will be notified to resubmit.
          </p>

          <div className="space-y-2">
            <label htmlFor="kyb-reject-reason" className="text-sm font-medium text-[color:var(--text-primary)]">
              Rejection reason
            </label>
            <select
          ...
          <div className="space-y-2">
            <label htmlFor="kyb-reject-notes" className="text-sm font-medium text-[color:var(--text-primary)]">
              Notes (optional)
            </label>
            <textarea
              id="kyb-reject-notes"
          ...
          <div className="rounded-lg border border-[color:var(--border-default)] bg-[color:var(--bg-muted)] px-4 py-4">
            <p className="text-sm leading-relaxed text-[color:var(--text-light)]">
              Rejecting a verification cannot be undone. The business will need to complete the KYB
              flow again.
            </p>
          </div>
