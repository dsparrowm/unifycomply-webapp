...
export function KycEscalateModal({ open, detail, onClose, onConfirm }: KycEscalateModalProps) {
  useEffect(() => {
    if (!open) {
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="kyc-escalate-title"
      ...
      >
        <div className="flex items-center justify-between border-b border-[color:var(--border-default)] px-6 py-5">
          <h2 id="kyc-escalate-title" className="text-lg font-semibold text-[color:var(--text-primary)]">
        ...
        >
          <p className="text-sm text-[color:var(--text-primary)]">
            Escalate <span className="font-medium">{detail.customerName}</span> ({detail.kycId}) to a
          ...
          <div className="space-y-2">
            <label htmlFor="escalate-notes" className="text-sm font-medium text-[color:var(--text-primary)]">
              Escalation notes
            </label>
            <textarea
              id="escalate-notes"
