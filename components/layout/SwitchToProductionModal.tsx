...
type SwitchToProductionModalProps = {
  open: boolean;
  onClose: () => void;
  onSwitch: () => void;
};

export function SwitchToProductionModal({
  open,
  onClose,
  onSwitch,
}: SwitchToProductionModalProps) {
  useEffect(() => {
    if (!open) {
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="switch-production-title"
      ...
      >
        ...
        <div className="flex flex-col items-center px-6 py-10 text-center">
          ...
          <p className="mt-2 max-w-sm text-sm text-[color:var(--text-muted)]">
            Wallet funding is only available for production account
          </p>
        </div>
