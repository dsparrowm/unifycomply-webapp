...
const statusFilterMap: Record<Exclude<KycStatusFilter, "all">, KybRecord["status"]> = {
  success: "approved",
  pending: "pending",
  "in-review": "in-review",
  failed: "rejected",
...
const priorityFilterMap: Record<
  Exclude<KybListFilters["priority"], "all">,
  KybRecord["priority"]
> = {
  urgent: "critical",
  high: "high",
...
function isWithinDateFilter(submittedAt: string, dateFilter: KybListFilters["date"]) {
...
export function filterKybRecords(records: KybRecord[], filters: KybListFilters) {
